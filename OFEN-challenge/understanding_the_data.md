...
### Daily production totals for 2026
...
' then cut -f datum,energietraeger,produktion_gwh   then rename 'produktion_gwh,Produktion_GWh'   --ocsv then skip-trivial-records stromproduktion.json > stromproduktion.csv
```

and compare to be sure we have the same numbers, i.e. no errors during the JSON
...
Now we can compare the two sources and verify they refer to the same
quantities.
...
mlr --csv \
join --lp Daily_ --rp Hourly_ -j Type -r MatchType -f stromproduktion_2026_01_GWh.csv entsoe_GWh.csv \
|mlr --c2p reorder -f Type,Hourly_Type,Daily_Production_GWh,Hourly_Production_GWh \
then rename Type,SFOE_Type,Hourly_Type,ENTSOE_Type \
then put '
  $Difference = $Daily_Production_GWh - $Hourly_Production_GWh;
  $Percentage = ($Daily_Production_GWh == 0 ? 0 : 100 * $Difference / $Daily_Production_GWh)
...
then reorder -f SFOE_Type,ENTSOE_Type  type_and_production.csv
```
...
Speicherkraft,Hydro: Pumped Storage + Water Reservoir,1743.0238399999992
```
...
```
mlr --csv filter '$Type != "Speicherkraft"' \
then rename Type,SFOE_Type,Hourly_Type,ENTSOE_Type \
then put '
  $Abs_Diff_GWh = $DailyProduction_GWh - $HourlyProduction_GWh;
  $Rel_Diff_pct = ($DailyProduction_GWh == 0 ? 0 : 100 * $Abs_Diff_GWh / $DailyProduction_GWh)
' \
then reorder -f SFOE_Type,ENTSOE_Type,Daily_Production_GWh,Hourly_Production_GWh,Difference,Percentage \
type_and_production.csv
...
```
SFOE_Type,ENTSOE_Type,DailyProduction_GWh,HourlyProduction_GWh,Abs_Diff_GWh,Rel_Diff_pct
Flusskraft,Hydro Run-of-river and poundage,706.0999999999999,661.7663986239978,44.33360137600209,6.278657608837573
Kernkraft,Nuclear,1454.1000000000001,1458.4139899999996,-4.313989999999421,-0.2966776700364088
Photovoltaik,Solar,205.89999999999998,78.35501765500003,127.54498234499995,61.945110415250106
Wind,Wind Onshore,17.999999999999996,17.98906078800001,0.010939211999986043,0.06077339999992247
...
We need to get the `DailyProduction_GWh` column back in !
...
```
SFOE_Type,ENTSOE_Type,Daily_Production_GWh,Hourly_Production_GWh,Difference,Percentage
Speicherkraft,Hydro: Pumped Storage + Water Reservoir,2177.1000000000004,1743.0238399999992,434.0761600000012,19.938273850535168
```

Actually save the output as a new CSV file.
...
```
mlr --csv cat \
then sort -f SFOE_Type type_and_production_without_Speicherkraft.csv type_and_production_only_Speicherkraft.csv
```
...
Actually save the output as a new CSV file `type_and_production_harmonised.csv`.
